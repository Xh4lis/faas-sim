    ...
    def setup(self, env):
        ...
        if hasattr(env, 'container_registry'):
            ...
            env.container_registry.register_image(
                ...
                ImageProperties('resnet50-inference-cpu', 56000000, 'latest', 'aarch64')
            )
            
            env.container_registry.register_image(
                ImageProperties('resnet50-inference-gpu', 56000000, 'latest', 'arm32'),
                ImageProperties('resnet50-inference-gpu', 56000000, 'latest', 'x86'),
                ImageProperties('resnet50-inference-gpu', 56000000, 'latest', 'aarch64')
            )
        ...
        if hasattr(env, 'faas'):
            ...
            resnet_deployment = FunctionDeployment(
                ...
                scaling_config=ScalingConfiguration(scale_min=1, scale_max=5, target_concurrency=10)
            )
            ...
            compute_deployment = FunctionDeployment(
                ...
                scaling_config=ScalingConfiguration(scale_min=1, scale_max=5, target_concurrency=5)
            )
...
def create_complex_topology():
    ...
    for i in range(5):
        topology.add_node(f'edge_dc_{i}', {
...
def main():
    ...
    logger.info(f"Created topology with {len(topology.nodes)} nodes and {len(topology.edges)} connections")
    ...
    image_recog_deployment = FunctionDeployment(
        ...
        scaling_config=ScalingConfiguration(scale_min=2, scale_max=10, target_concurrency=5)
    )
    ...
    data_proc_deployment = FunctionDeployment(
        ...
        scaling_config=ScalingConfiguration(scale_min=1, scale_max=5, target_concurrency=3)
    )
    ...
    api_gateway_deployment = FunctionDeployment(
        ...
        scaling_config=ScalingConfiguration(scale_min=4, scale_max=12, target_concurrency=10)
    )
    ...
    video_analytics_deployment = FunctionDeployment(
        ...
        scaling_config=ScalingConfiguration(scale_min=1, scale_max=3, target_concurrency=2)
    )
...
def analyze_results(env):
    ...
    with open(f"{output_dir}/summary.txt", 'w') as f:
        ...
        if 'invocations_df' in dfs and dfs['invocations_df'] is not None:
            ...
            f.write(f"Simulation duration: {inv_df['t_start'].max() - inv_df['t_start'].min():.2f} s\n\n")
            ...
            for func in inv_df['function'].unique():
                ...
                f.write(f"    Avg execution time: {func_data['duration'].mean():.4f} s\n")
                if 'wait_time' in func_data.columns:
                    f.write(f"    Avg wait time: {func_data['wait_time'].mean():.4f} s\n")
                f.write("\n")
