...
class ReportGenerator:
    ...
    def load_data(self):
        ...
        if 'invocations_df' in self.data and not self.data['invocations_df'].empty:
            ...
            self.functions = df['function_name'].unique() if 'function_name' in df.columns else []
            self.images = df['function_image'].unique() if 'function_image' in df.columns else []
            
            print(f"Found {len(self.nodes)} nodes, {len(self.functions)} functions, and {len(self.images)} images")
    ...
    def generate_function_invocation_report(self):
        ...
        for i, node in enumerate(self.nodes):
            ...
            axes[0, 1].scatter(node_df['t_start'], node_df['t_exec'], 
                             alpha=0.6, color=colors[i % len(colors)], label=node)
        ...
        for i, function in enumerate(self.functions):
            ...
            axes[1, 0].step(start_times, cum_invocations, where='post', 
                          color=colors[i % len(colors)], label=function)
        ...
        for i, node in enumerate(self.nodes):
            node_df = inv_df[inv_df['node'] == node]
            axes[1, 1].scatter(node_df['t_wait'], node_df['t_exec'], 
                             alpha=0.6, color=colors[i % len(colors)], label=node)
        axes[1, 1].set_title('Wait Time vs Execution Time by Node')
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.output_dir, 'function_invocation_report.png'), dpi=300)
        plt.close()
        
        # Generate the summary text file
        self._generate_invocation_summary_text(inv_df)
    
    def _generate_invocation_summary_text(self, inv_df):
        ...
        with open(os.path.join(self.output_dir, 'invocation_summary.txt'), 'w') as f:
            ...
            f.write(f"Overall average execution time: {inv_df['t_exec'].mean():.2f} ms\n")
            f.write(f"Overall average wait time: {inv_df['t_wait'].mean():.2f} ms\n")
            f.write(f"Simulation start time: {inv_df['t_start'].min():.2f} s\n")
            f.write(f"Simulation end time: {inv_df['t_start'].max() + inv_df['t_exec'].max()/1000:.2f} s\n")
            f.write(f"Total duration: {inv_df['t_start'].max() - inv_df['t_start'].min():.2f} s\n\n")
            ...
            if len(self.nodes) > 0:
                ...
                for node in self.nodes:
                    node_df = inv_df[inv_df['node'] == node]
                    f.write(f"Node: {node}\n")
                    f.write(f"  Invocations: {len(node_df)}\n")
                    f.write(f"  Average execution time: {node_df['t_exec'].mean():.2f} ms\n")
                    f.write(f"  Min/Max execution time: {node_df['t_exec'].min():.2f}/{node_df['t_exec'].max():.2f} ms\n")
                    f.write(f"  Average wait time: {node_df['t_wait'].mean():.2f} ms\n")
                    f.write("\n")
            ...
            if len(self.functions) > 0:
                ...
                for function in self.functions:
                    ...
                    f.write(f"  Invocations: {len(func_df)}\n")
                    f.write(f"  Average execution time: {func_df['t_exec'].mean():.2f} ms\n")
                    f.write(f"  Min/Max execution time: {func_df['t_exec'].min():.2f}/{func_df['t_exec'].max():.2f} ms\n")
                    f.write(f"  Average wait time: {func_df['t_wait'].mean():.2f} ms\n")
                    f.write("\n")
            ...
            for (node, fn_name, fn_image), group in grouped:
                ...
                f.write(f"Min/Max execution time: {group['t_exec'].min():.2f}/{group['t_exec'].max():.2f} ms\n")
                f.write(f"Average wait time: {group['t_wait'].mean():.2f} ms\n")
    ...
    def generate_resource_utilization_report(self):
        ...
        if 'function_utilization_df' not in self.data or self.data['function_utilization_df'].empty:
        ...
        if len(nodes) > 0:
            ...
            for i, node in enumerate(nodes):
                ...
                if not node_df.empty:
                    ...
                    axes[0].plot(x_values, node_df['cpu_util'], marker='.', linestyle='-', 
                               color=colors[i % len(colors)], label=f"{node}")
            ...
            for i, node in enumerate(nodes):
                ...
                axes[0].axhline(y=avg_cpu, color=colors[i % len(colors)], linestyle='--', alpha=0.5)
        else:
            # No node information, just plot overall CPU utilization
            x_values = range(len(util_df))
            axes[0].plot(x_values, util_df['cpu_util'], marker='.', linestyle='-', color='blue')
            ...
            axes[0].axhline(y=avg_cpu, color='red', linestyle='--', 
                          label=f'Avg: {avg_cpu:.2f}%')
            axes[0].legend()
        
        # Plot average CPU utilization by node as bar chart
        if len(nodes) > 0:
            node_avg_cpu = util_df.groupby('node')['cpu_util'].mean()
            axes[1].bar(node_avg_cpu.index, node_avg_cpu.values, 
            ...
            axes[1].plot(x_values, util_df['cpu']/1000, marker='s', linestyle='-', 
                       color='green', label='CPU Allocation')
            ...
            axes[1].axhline(y=avg_cpu_alloc, color='red', linestyle='--', 
                          label=f'Avg: {avg_cpu_alloc:.2f} units')
            axes[1].legend()
        
        plt.tight_layout()
        plt.savefig(os.path.join(self.output_dir, 'function_utilization_report.png'), dpi=300)
        plt.close()
    ...
    def _generate_utilization_summary_text(self, util_df, nodes):
        ...
        with open(os.path.join(self.output_dir, 'utilization_summary.txt'), 'w') as f:
            ...
            f.write("===========================\n\n")
            f.write(f"Overall average CPU utilization: {util_df['cpu_util'].mean():.2f}%\n")
            ...
            if len(nodes) > 0:
                ...
                for node in nodes:
                    node_df = util_df[util_df['node'] == node]
                    f.write(f"Node: {node}\n")
                    f.write(f"  Average CPU utilization: {node_df['cpu_util'].mean():.2f}%\n")
                    f.write(f"  Peak CPU utilization: {node_df['cpu_util'].max():.2f}%\n")
                    
                    if 'cpu' in node_df.columns:
                        f.write(f"  CPU allocation: {node_df['cpu'].mean()/1000:.2f} CPU units\n")
                    ...
                    if 'function_name' in node_df.columns:
                        ...
                        for function, func_df in function_groups:
                            f.write(f"  Function {function}:\n")
                            f.write(f"    Average CPU: {func_df['cpu_util'].mean():.2f}%\n")
    ...
    def generate_deployment_timeline(self):
        ...
        if 'function_deployment_lifecycle_df' in self.data and not self.data['function_deployment_lifecycle_df'].empty:
        ...
        if 'replica_deployment_df' in self.data and not self.data['replica_deployment_df'].empty:
            for i, row in enumerate(self.data['replica_deployment_df'].itertuples()):
                timestamp = getattr(row, 'timestamp', i) if hasattr(row, 'timestamp') else i
                lifecycle_events.append({
                    'time': timestamp,
        ...
        if 'flow_df' in self.data and not self.data['flow_df'].empty:
            for j, row in enumerate(self.data['flow_df'].itertuples()):
                timestamp = getattr(row, 'timestamp', 2 + j/10) if hasattr(row, 'timestamp') else 2 + j/10
                lifecycle_events.append({
                    'time': timestamp,
                    'event': f"{row.action_type} ({row.bytes/1024/1024:.1f} MB)",
                    'type': 'network',
                    'node': f"{row.source} → {row.sink}",
                    'function': getattr(row, 'function_name', 'unknown') if hasattr(row, 'function_name') else 'unknown'
                })
        ...
        for i, group in enumerate(sorted_groups):
            y_positions[group] = (num_groups - i - 1) * 2  # Reverse order and double spacing
        ...
        plt.xlim(-0.5, max([e['time'] for e in lifecycle_events]) + 5)  # Add padding on right for labels
        
        # Add legend
        from matplotlib.lines import Line2D
        legend_elements = [
            Line2D([0], [0], color=event_colors['deployment'], lw=4, label='Deployment Events'),
            Line2D([0], [0], color=event_colors['replica'], lw=4, label='Replica Events'),
            Line2D([0], [0], color=event_colors['network'], lw=4, label='Network Events')
        ]
    ...
    def generate_node_comparison_report(self):
        ...
        node_means = [inv_df[inv_df['node'] == node]['t_exec'].mean() for node in self.nodes]
        ...
        # Plot 4: Wait time by node
        node_wait_means = [inv_df[inv_df['node'] == node]['t_wait'].mean() for node in self.nodes]
        axes[1, 1].bar(self.nodes, node_wait_means, color='coral')
        axes[1, 1].set_title('Average Wait Time by Node')
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.output_dir, 'node_comparison_report.png'), dpi=300)
        plt.close()
        ...
        if len(self.functions) > 1:
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(self.output_dir, 'function_distribution.png'), dpi=300)
            plt.close()
    ...
    def generate_function_comparison_report(self):
        ...
        function_data = [inv_df[inv_df['function_name'] == fn]['t_exec'] for fn in self.functions]
        ...
        # Plot 2: Bar chart of average execution times
        function_means = [inv_df[inv_df['function_name'] == fn]['t_exec'].mean() for fn in self.functions]
        ...
        axes[1, 0].bar(function_counts.index, function_counts.values, color='lightgreen')
        ...
        # Plot 4: Wait time by function
        function_wait_means = [inv_df[inv_df['function_name'] == fn]['t_wait'].mean() for fn in self.functions]
        axes[1, 1].bar(self.functions, function_wait_means, color='coral')
        axes[1, 1].set_title('Average Wait Time by Function')
        axes[1, 1].set_ylabel('Average Wait Time (ms)')
        axes[1, 1].grid(True, axis='y', linestyle='--', alpha=0.7)
        axes[1, 1].set_xticklabels(self.functions, rotation=45)
        
        plt.tight_layout()
        plt.savefig(os.path.join(self.output_dir, 'function_comparison_report.png'), dpi=300)
        plt.close()
        ...
        if len(self.nodes) > 0 or len(self.functions) > 0 or len(self.images) > 0:
            ...
            if len(self.nodes) > 1 and 'invocations_df' in self.data:
                ...
                for node in self.nodes:
                    ...
                    summary_text.append(f"  {node}: {node_count} invocations ({node_count/len(inv_df)*100:.1f}%)")
                summary_text.append("")
            ...
            if len(self.functions) > 1 and 'invocations_df' in self.data:
                ...
                for function in self.functions:
                    ...
                    summary_text.append(f"  {function}: {func_count} invocations ({func_count/len(inv_df)*100:.1f}%)")
                summary_text.append("")
        
        # Add invocation statistics
        if 'invocations_df' in self.data and not self.data['invocations_df'].empty:
            inv_df = self.data['invocations_df']
            summary_text.extend([
                ...
                f"  Avg wait time: {inv_df['t_wait'].mean():.2f} ms",
                f"  Max wait time: {inv_df['t_wait'].max():.2f} ms",
                f"  Simulation duration: {inv_df['t_start'].max() - inv_df['t_start'].min():.2f} s",
                "",
            ])
        
        # Add resource utilization
        if 'function_utilization_df' in self.data and not self.data['function_utilization_df'].empty:
            util_df = self.data['function_utilization_df']
            summary_text.extend([
            ...
            if len(self.nodes) > 1:
                for node in self.nodes:
                    ...
                    if not node_df.empty:
                        ...
                        summary_text.append(f"    Avg CPU: {node_df['cpu_util'].mean():.2f}%")
                        summary_text.append(f"    Peak CPU: {node_df['cpu_util'].max():.2f}%")
                summary_text.append("")
            ...
            if 'source' in flow_df.columns and 'sink' in flow_df.columns:
                ...
                if not flow_pairs.empty:
                    ...
                    for (source, sink), row in flow_pairs.iterrows():
                        summary_text.append(f"  {source} → {sink}: {row['bytes']/1024/1024:.2f} MB in {row['duration']:.2f} s")
                    summary_text.append("")
...
def main():
    ...
    report_gen = ReportGenerator(data_dir='analysis_data', output_dir='visualization_results')
