...
class PythonPiSimulator(RealisticFunctionSimulator):
    ...
    def invoke(
        ...
        exec_time, cpu_util = self._calculate_execution_metrics(request)
        logger.info(
            f"PythonPi execution time: {exec_time*1000:.2f}ms, CPU: {cpu_util:.1f}%"
        )
        yield env.timeout(exec_time)
        return
...
class ResNet50Simulator(RealisticFunctionSimulator):
    ...
    def invoke(
        ...
        exec_time, cpu_util = self._calculate_execution_metrics(request)
        logger.info(
            f"ResNet50 execution time: {exec_time*1000:.2f}ms, CPU: {cpu_util:.1f}%"
        )
        yield env.timeout(exec_time)
        return
...
class EnhancedBenchmark(Benchmark):
        ...
        while env.now < phase_end:
            ...
            for _ in range(num_requests):
                ...
                if random.random() < 0.6:
                    ...
                    req = FunctionRequest(
                        "resnet50-inference", size=2.0 + batch_size * 10
                    )
                    processes.append(env.process(env.faas.invoke(req)))
        ...
        while env.now < phase_end:
            ...
            for _ in range(num_requests):
                ...
                if random.random() < 0.5:
                    ...
                    req = FunctionRequest(
                        "resnet50-inference", size=image_size + batch_size * 10
                    )
                    processes.append(env.process(env.faas.invoke(req)))
            ...
            if random.random() < 0.2:
                ...
                while env.now < burst_end and env.now < phase_end:
                    ...
                    for edge_node in random.sample(edge_nodes, min(3, len(edge_nodes))):
                        for cloud_node in random.sample(
                            cloud_nodes, min(2, len(cloud_nodes))
                        ):
                            yield env.process(
                                self._simulate_image_transfer(
                                    env,
                    ...
                    for _ in range(num_requests):
                        ...
                        if random.random() < 0.4:
                            ...
                            req = FunctionRequest(
                                "resnet50-inference", size=4.0 + batch_size * 10
                            )
                            processes.append(env.process(env.faas.invoke(req)))
    ...
    def _simulate_image_transfer(self, env, source, destination, size_mb):
        ...
        try:
            ...
            if hasattr(env.topology, "route_by_node_name"):
                ...
                if "sim.net" in sys.modules and hasattr(
                    sys.modules["sim.net"], "SafeFlow"
                ):
    ...
    def prepare_python_pi_deployment(self):
        ...
        python_pi_fd = FunctionDeployment(
            python_pi_fn, [python_pi_fn_container], scaling_config
        )
