...
from sim.oracle.data.distributions import (
...
class Oracle:
    ...
    def estimate(
        self, context: ClusterContext, pod: Pod, scheduling_result: SchedulingResult
    ) -> Tuple[str, str]:
...
class EmpiricalOracle:
    def __init__(self, filename):
        ...
        df["bandwidth"] = df["bandwidth"].apply(
            lambda x: 1.25e8 if x is None else parse_size_string(f"{x.mbit}M") / 8
        )
...
class StartupTimeOracle(EmpiricalOracle):
    def __init__(self):
        super(StartupTimeOracle, self).__init__(
            os.path.join(data_dir, "pod_startup_*.csv")
        )
        self.durations = self.dataset[
            ["host", "bandwidth", "image", "image_present", "duration"]
        ]
    ...
    def estimate(
        ...
        for container in pod.spec.containers:
            ...
            image_present = (
                normalize_image_name(image) not in scheduling_result.needed_images
            )

            data = self.durations.query(
                f'host == "{host_type}" and '
                f'image == "{image}" and '
...
class ExecutionTimeOracle(EmpiricalOracle):
    def __init__(self):
        super(ExecutionTimeOracle, self).__init__(
            os.path.join(data_dir, "exec_time*.csv")
        )
    ...
    def estimate(
        ...
        bandwidth = context.get_bandwidth_graph()[host][
            context.get_next_storage_node(scheduling_result.suggested_host)
        ]
        execution_time = 0
        for container in pod.spec.containers:
            image = container.image
            execution_time += (
                self.durations.query(
                    f'host == "{host_type}" and '
                    f"bandwidth == {bandwidth} and "
                    f'image == "{image}"'
                )["duration"]
...
class CostOracle(Oracle):
    ...
    def __init__(self, execution_time_oracle=None) -> None:
        super().__init__()
        self.execution_time_oracle = (
            execution_time_oracle or FittedExecutionTimeOracle()
        )
    ...
    def estimate(
        ...
        if (
            "locality.skippy.io/type" in labels
            and labels["locality.skippy.io/type"] == "cloud"
        ):
            _, time_str = self.execution_time_oracle.estimate(
                context, pod, scheduling_result
            )
...
class ResourceUtilizationOracle(Oracle):
    def estimate(
        ...
        if (
            "locality.skippy.io/type" in labels
            and labels["locality.skippy.io/type"] == "edge"
        ):
    ...
    def score_resource_utilization(self, pod, node) -> float:
        ...
        cpu_all = 0
        for container in pod.spec.containers:
            cpu_all += container.resources.requests.get(
                "cpu", container.resources.default_milli_cpu_request
            )
            mem_all += container.resources.requests.get(
...
class FittedStartupTimeOracle(Oracle):
    ...
    def __init__(self) -> None:
        ...
        self.startup_time_samplers = {
            ...
            for k, (xmin, xmax, dist) in startup_time_distributions.items()
        }
