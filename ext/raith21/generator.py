...
def xeon_reqs():
    xeon_single_device_req = Requirements(
        arch={Arch.X86: 1},
        accelerator={Accelerator.NONE: 1},
        cores={Bins.LOW: 1},
        disk={Disk.SSD: 1},
        location={Location.CLOUD: 1},
        connection={Connection.ETHERNET: 1},
        network={Bins.LOW: 1},
        cpu_mhz={Bins.LOW: 1},
        cpu={CpuModel.XEON: 1},
        ram={Bins.LOW: 1},
...
def create_generator(arches, t, heterogeneity_score, base_req, folder):
    ...
    for index, arch in enumerate(arches):
        ...
        settings = list(
            map(lambda i: create_t_setting(i, t[index]), range(len(t[index])))
        )
...
def create_settings(
    ...
    base_req,
    tuples,
    heterogeneity_score: Callable[[Requirements, Requirements], float],
    folder: str,
):
    ...
    list(
        (
            map(
                lambda t: create_generator(
                    arches, t, heterogeneity_score, base_req, folder
                ),
                product,
            )
        )
...
def create_and_save_settings(
    arches, base_requirement, tuples, heterogeneity_score, folder
):
...
def process_arches(arch_probs, probs_for_archs, base_req, heterogeneity_score, folder):
    for arches in arch_probs:
        ...
        create_and_save_settings(arches, base_req, tuples, heterogeneity_score, folder)


def filter_invalid_settings(old_probs_per_arch):
    # no gpu/tpu arm32
    old_probs_per_arch[Arch.ARM32]["accelerator"] = (
        (Accelerator.NONE, 1),
        (Accelerator.GPU, 0),
        (Accelerator.TPU, 0),
    )
    ...
    old_probs_per_arch[Arch.ARM32]["cpu"] = (
        (CpuModel.I7, 0),
        (CpuModel.XEON, 0),
        (CpuModel.ARM, 1),
    )
    old_probs_per_arch[Arch.AARCH64]["cpu"] = (
        (CpuModel.I7, 0),
        (CpuModel.XEON, 0),
        (CpuModel.ARM, 1),
    )
    ...
    old_probs_per_arch[Arch.ARM32]["ram"] = list(
        filter(lambda f: f[3][1] == 0, old_probs_per_arch[Arch.ARM32]["ram"])
    )
    # filter high ram
    old_probs_per_arch[Arch.ARM32]["ram"] = list(
        filter(lambda f: f[2][1] == 0, old_probs_per_arch[Arch.ARM32]["ram"])
    )
    # filter high cores
    old_probs_per_arch[Arch.ARM32]["cores"] = list(
    ...
    # filter arm cpu
    old_probs_per_arch[Arch.X86]["cpu"] = list(
        filter(lambda f: f[2][1] == 0, old_probs_per_arch[Arch.X86]["cpu"])
    )
    # filter maxwell
...
def generate_settings(
    ...
    folder = (
        f"{folder}/{now}_archsteps-{arch_steps}_steps-{steps}_percentage-{percentage}"
    )
    ...
    for arch, proportion in settings.arch.items():
        for i in range(int(n * proportion)):
            characteristics = list(
                map(
                    lambda x: get_property_with_probs(x),
                    settings.properties[arch].values,
                )
            )
