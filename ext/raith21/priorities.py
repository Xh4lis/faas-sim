...
class CapabilityMatchingPriority(Priority):
    def map_node_score(self, context: ClusterContext, pod: Pod, node: Node) -> int:
        ...
        node_caps = dict(
            filter(lambda label: "device.edgerun.io" in label[0], node.labels.items())
        )
    ...
    def reduce_mapped_score(
        self, context: ClusterContext, pod: Pod, nodes: [Node], node_scores: [int]
    ) -> [int]:
    ...
    def map_node_score(self, context: ClusterContext, pod: Pod, node: Node) -> int:
        ...
        for running_pod in node.pods:
            ...
            if usage != None:
                ...
                counter += 1
        if counter > 0:
            running_net /= self._get_net_speed(
                node.labels.get("device.edgerun.io/location")
            )
            running_blkio /= self._get_disk_speed(
                node.labels.get("device.edgerun.io/disk")
            )
